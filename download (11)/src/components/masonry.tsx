...
const useMedia = (
  ...
  }, [queries, values, defaultValue, isBrowser]);
...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const handleMouseEnter = (id: string) => {
    ...
    const el = containerRef.current.querySelector(`[data-key="${id}"]`);
    if(!el) return;
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full opacity-0" style={{ opacity: imagesReady ? 1 : 0, transition: 'opacity 0.5s' }}>
      {grid.map((item) => (
        <div
          key={item.id}
          data-key={item.id}
